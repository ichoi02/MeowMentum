#!/usr/bin/env bash
# Compile and flash test/back_pd_rotate to the back Teensy (SN_BACK).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_back_pd_rotate.sh
#   bash test/flash_back_pd_rotate.sh --compile-only
#   bash test/flash_back_pd_rotate.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_back_pd_rotate.sh
#
...
usage() {
  cat <<'EOF'
Usage: bash test/flash_back_pd_rotate.sh [options]

  (default)      Compile back_pd_rotate, then flash to SN_BACK.
  ...
  BACK_PD_ROTATE_BUILD_DIR=...   Build output root (default: test/.back_pd_rotate_build)
EOF
}
...
SKETCH="${SCRIPT_DIR}/back_pd_rotate"
INO="${SKETCH}/back_pd_rotate.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.back_pd_rotate_build"
OUT_DIR="${BACK_PD_ROTATE_BUILD_DIR:-$DEFAULT_BUILD_ROOT}"
HEX="${OUT_DIR}/back_pd_rotate.ino.hex"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Back PD rotate: FQBN=$FQBN -> $OUT_DIR"
...
if [[ "$DO_FLASH" -eq 0 ]]; then
  ...
  echo "Flash later with: bash test/flash_back_pd_rotate.sh --flash-only"
  exit 0
fi
...
echo "Done. Back -> ${INO}"
echo "Command from Pi:"
echo "  python test/cmd_back_pd_rotate.py m2 0.2"
echo "  python test/cmd_back_pd_rotate.py m1 -0.1"
echo "  python test/cmd_back_pd_rotate.py zero"
