#!/usr/bin/env bash
# Compile and flash test/front_sweep_45 to the front Teensy (SN_FRONT).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_front_sweep_45.sh
#   bash test/flash_front_sweep_45.sh --compile-only
#   bash test/flash_front_sweep_45.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_front_sweep_45.sh
#
# teensy_loader_cli cannot select by USB serial — press the program button on
# the front board only before flashing.

set -euo pipefail

usage() {
  cat <<'EOF'
Usage: bash test/flash_front_sweep_45.sh [options]

  (default)      Compile front_sweep_45, then flash to SN_FRONT.
  ...
  FRONT_SWEEP_45_BUILD_DIR=...   Build output root (default: test/.front_sweep_45_build)
EOF
}
...
SKETCH="${SCRIPT_DIR}/front_sweep_45"
INO="${SKETCH}/front_sweep_45.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.front_sweep_45_build"
OUT_DIR="${FRONT_SWEEP_45_BUILD_DIR:-$DEFAULT_BUILD_ROOT}"
HEX="${OUT_DIR}/front_sweep_45.ino.hex"
...
SN_FRONT="$(python3 - "$CONTROLLER_PY" <<'PY'
import re, sys
text = open(sys.argv[1], encoding="utf-8").read()
m = re.search(r'^SN_FRONT\s*=\s*["\']([^"\']+)["\']', text, re.MULTILINE)
if not m:
    raise SystemExit(f"Could not parse SN_FRONT from {sys.argv[1]}")
print(m.group(1))
PY
)"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Front 45deg sweep: FQBN=$FQBN -> $OUT_DIR"
...
echo "Front hex: $HEX ($(wc -c <"$HEX") bytes)"

if [[ "$DO_FLASH" -eq 0 ]]; then
  echo "Compile-only finished."
  echo "Flash later with: bash test/flash_front_sweep_45.sh --flash-only"
  exit 0
fi
...
echo ">>> Front (USB serial ${SN_FRONT}) <<<"
echo "    Firmware: $(basename "$HEX")"
echo "    teensy_loader_cli flashes WHICHEVER board is in bootloader — it does NOT select by SN."
echo "    1) Unplug the BACK Teensy USB if unsure, OR"
echo "    2) Press program ONLY on the board that shows SN=${SN_FRONT} in:"
...
echo "Done. Front -> ${INO}"
echo "Capture + plot:"
echo "  python test/log_front_sweep_45.py"
echo "  python test/log_front_sweep_45.py --motor m1"
echo "  python test/log_front_sweep_45.py --motor m2"
