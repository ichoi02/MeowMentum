#!/usr/bin/env bash
# Compile and flash test/encoder_check_front to the front Teensy (SN_FRONT).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_encoder_check_front.sh
#   bash test/flash_encoder_check_front.sh --compile-only
#   bash test/flash_encoder_check_front.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_encoder_check_front.sh
#
...
usage() {
  cat <<'EOF'
Usage: bash test/flash_encoder_check_front.sh [options]

  (default)      Compile encoder_check_front, then flash to SN_FRONT.
  ...
  ENCODER_CHECK_BUILD_DIR=...   Build output root (default: test/.encoder_check_build)
EOF
}
...
SKETCH="${SCRIPT_DIR}/encoder_check_front"
INO="${SKETCH}/encoder_check_front.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.encoder_check_build"
OUT_DIR="${ENCODER_CHECK_BUILD_DIR:-$DEFAULT_BUILD_ROOT}/front"
HEX="${OUT_DIR}/encoder_check_front.ino.hex"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Front encoder check: FQBN=$FQBN -> $OUT_DIR"
...
if [[ "$DO_FLASH" -eq 0 ]]; then
  ...
  echo "Flash later with: bash test/flash_encoder_check_front.sh --flash-only"
  exit 0
fi
...
echo "Done. Front -> ${INO}"
echo "Watch encoders:"
echo "  python test/monitor_teensy.py --front"
