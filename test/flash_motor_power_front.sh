#!/usr/bin/env bash
# Compile and flash test/motor_power_front to the front Teensy (SN_FRONT).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_motor_power_front.sh
#   bash test/flash_motor_power_front.sh --compile-only
#   bash test/flash_motor_power_front.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_motor_power_front.sh
#
...
usage() {
  cat <<'EOF'
Usage: bash test/flash_motor_power_front.sh [options]

  (default)      Compile motor_power_front, then flash to SN_FRONT.
...
SKETCH="${SCRIPT_DIR}/motor_power_front"
INO="${SKETCH}/motor_power_front.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.motor_power_build"
OUT_DIR="${MOTOR_POWER_BUILD_DIR:-$DEFAULT_BUILD_ROOT}/front"
HEX="${OUT_DIR}/motor_power_front.ino.hex"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Front motor power: FQBN=$FQBN -> $OUT_DIR"
...
if [[ "$DO_FLASH" -eq 0 ]]; then
  ...
  echo "Flash later with: bash test/flash_motor_power_front.sh --flash-only"
  exit 0
fi
