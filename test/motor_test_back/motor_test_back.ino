 ...
 *
 * Flash to the back board. Watch with: python test/monitor_teensy.py --back
 */
...
void setup() {
  ...
  Serial.println("back DEBUG: encoder readout only (drivers EN=HIGH, motors coast)");
  Serial.println("m1_ticks,m2_ticks,m1_rad,m2_rad,m1A,m1B,m2A,m2B");
  Serial.println("If pins never change when turning by hand: check motor battery power + you are on BACK joints.");
  Serial.println("Running 400ms strong nudge on M1 then M2 (watch BACK joints move)...");
  ...
  Serial.println("motor_test_back: starting in 2s...");
  delay(2000);
  digitalWrite(M1EN, HIGH);
  digitalWrite(M2EN, HIGH);
#endif
...
void loop() {
  ...
  Serial.print(',');
  Serial.print(digitalRead(13));  // enc1 A
  Serial.print(',');
  Serial.print(digitalRead(14));  // enc1 B
  Serial.print(',');
  Serial.print(digitalRead(15));  // enc2 A
  Serial.print(',');
  Serial.println(digitalRead(16)); // enc2 B
#else
  Serial.println("--- Back motor sweep ---");
