#!/usr/bin/env bash
# Compile and flash test/back_dir_power to the back Teensy (SN_BACK).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_back_dir_power.sh
#   bash test/flash_back_dir_power.sh --compile-only
#   bash test/flash_back_dir_power.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_back_dir_power.sh
#
...
usage() {
  cat <<'EOF'
Usage: bash test/flash_back_dir_power.sh [options]

  (default)      Compile back_dir_power, then flash to SN_BACK.
  ...
  BACK_DIR_POWER_BUILD_DIR=...   Build output root (default: test/.back_dir_power_build)
EOF
}
...
SKETCH="${SCRIPT_DIR}/back_dir_power"
INO="${SKETCH}/back_dir_power.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.back_dir_power_build"
OUT_DIR="${BACK_DIR_POWER_BUILD_DIR:-$DEFAULT_BUILD_ROOT}"
HEX="${OUT_DIR}/back_dir_power.ino.hex"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Back FWD/REV dir power: FQBN=$FQBN -> $OUT_DIR"
...
if [[ "$DO_FLASH" -eq 0 ]]; then
  ...
  echo "Flash later with: bash test/flash_back_dir_power.sh --flash-only"
  exit 0
fi
...
echo "Done. Back -> ${INO}"
echo "Sequence: M1 FWD, M1 REV, M2 FWD, M2 REV (400ms each @ 50% PWM)"
echo "Monitor: python test/monitor_teensy.py --back --raw"
