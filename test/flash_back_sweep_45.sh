#!/usr/bin/env bash
# Compile and flash test/back_sweep_45 to the back Teensy (SN_BACK).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_back_sweep_45.sh
#   bash test/flash_back_sweep_45.sh --compile-only
#   bash test/flash_back_sweep_45.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_back_sweep_45.sh
#
...
usage() {
  cat <<'EOF'
Usage: bash test/flash_back_sweep_45.sh [options]

  (default)      Compile back_sweep_45, then flash to SN_BACK.
  ...
  BACK_SWEEP_45_BUILD_DIR=...   Build output root (default: test/.back_sweep_45_build)
EOF
}
...
SKETCH="${SCRIPT_DIR}/back_sweep_45"
INO="${SKETCH}/back_sweep_45.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.back_sweep_45_build"
OUT_DIR="${BACK_SWEEP_45_BUILD_DIR:-$DEFAULT_BUILD_ROOT}"
HEX="${OUT_DIR}/back_sweep_45.ino.hex"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Back 45deg sweep: FQBN=$FQBN -> $OUT_DIR"
...
if [[ "$DO_FLASH" -eq 0 ]]; then
  ...
  echo "Flash later with: bash test/flash_back_sweep_45.sh --flash-only"
  exit 0
fi
...
echo "Done. Back -> ${INO}"
echo "Capture + plot:"
echo "  python test/log_back_sweep_45.py"
echo "  python test/log_back_sweep_45.py --motor m1"
echo "  python test/log_back_sweep_45.py --motor m2"
