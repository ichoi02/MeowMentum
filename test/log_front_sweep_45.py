#!/usr/bin/env python3
"""Capture front_sweep_45 serial CSV and plot encoder angles.

Firmware waits for RUN / RUN M1 / RUN M2 (does not auto-sweep on boot).

Usage:
  # Flash first, then:
  python test/log_front_sweep_45.py
  python test/log_front_sweep_45.py --motor m1
  python test/log_front_sweep_45.py --motor m2

  # Plot an existing CSV:
  python test/log_front_sweep_45.py --plot-only path/to/front_sweep_45_....csv
"""
...
def capture(path: str, out_csv: Path, motor: str, seconds: float = 30.0) -> Path:
    cmd = MOTOR_CMD[motor]
    print(f"Opening {path} @ {BAUD} (SN_FRONT={SN_FRONT})")
    ...
    if len(rows) == 0:
        raise SystemExit("No data rows captured. Reflash front_sweep_45 and retry.")
    return out_csv
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Log + plot front_sweep_45")
    parser.add_argument(
        "--motor",
        choices=("both", "m1", "m2"),
        default="both",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="CSV output path (default: test/logs/front_sweep_45_<motor>_<timestamp>.csv)",
    )
    ...
    if out is None:
        ...
        out = Path("test/logs") / f"front_sweep_45_{args.motor}_{stamp}.csv"
