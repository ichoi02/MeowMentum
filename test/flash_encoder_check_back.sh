#!/usr/bin/env bash
# Compile and flash test/encoder_check_back to the back Teensy (SN_BACK).
#
# Prerequisites: conda env cat + arduino-cli, teensy-loader-cli, PJRC udev rules.
#
# Usage:
#   conda activate cat
#   bash test/flash_encoder_check_back.sh
#   bash test/flash_encoder_check_back.sh --compile-only
#   bash test/flash_encoder_check_back.sh --flash-only
#   FQBN=teensy:avr:teensy41 bash test/flash_encoder_check_back.sh
#
...
usage() {
  cat <<'EOF'
Usage: bash test/flash_encoder_check_back.sh [options]

  (default)      Compile encoder_check_back, then flash to SN_BACK.
...
SKETCH="${SCRIPT_DIR}/encoder_check_back"
INO="${SKETCH}/encoder_check_back.ino"
DEFAULT_BUILD_ROOT="${SCRIPT_DIR}/.encoder_check_build"
OUT_DIR="${ENCODER_CHECK_BUILD_DIR:-$DEFAULT_BUILD_ROOT}/back"
HEX="${OUT_DIR}/encoder_check_back.ino.hex"
...
if [[ "$DO_COMPILE" -eq 1 ]]; then
  ...
  rm -rf "$OUT_DIR"
  mkdir -p "$OUT_DIR"
  echo "Compiling Back encoder check: FQBN=$FQBN -> $OUT_DIR"
...
if [[ "$DO_FLASH" -eq 0 ]]; then
  ...
  echo "Flash later with: bash test/flash_encoder_check_back.sh --flash-only"
  exit 0
fi
...
echo "Done. Back -> ${INO}"
echo "Watch encoders:"
echo "  python test/monitor_teensy.py --back"
