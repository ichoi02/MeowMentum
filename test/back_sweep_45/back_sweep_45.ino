/*
 * Back Teensy — one-shot ±45° sweep with encoder logging.
 * M1 = tail, M2 = back roll (same as hardware/controllerV2.py).
 *
 * Each motor: 45° CW, settle, then 45° CCW back to home.
 * Move aborts if not within deadband in MOVE_TIMEOUT_MS (1 s).
 *
 * Streams CSV for the whole run:
 ...
 *   RUN BOTH  — same
 *   RUN M1    — M1 (tail) only
 *   RUN M2    — M2 (back roll) only
 *
 * Flash: bash test/flash_back_sweep_45.sh
 * Log+plot: python test/log_back_sweep_45.py [--motor both|m1|m2]
 */
...
const uint32_t MOVE_TIMEOUT_MS = 1000;  // abort move if not converged
...
// Run PD + logging for durationMs (replaces bare delay so hold stays active).
...
void runSweepSequence(int mask) {
  ...
  if (doM1) {
    Serial.println("# EVENT sweep_m1_tail");
    sweepMotor(1, "M1_tail");
    activeMotor = 0;
    if (doM2) {
  ...
  if (doM2) {
    Serial.println("# EVENT sweep_m2_roll");
    sweepMotor(2, "M2_roll");
  }
...
void setup() {
  ...
  Serial.println("# FIRMWARE back_sweep_45");
