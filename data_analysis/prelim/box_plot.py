...
METRIC = 'F_rollrate_2p5m'
# METRIC = 'F_roll_2p5m'
METRIC = 'F_pitch_2p5m'
METRIC = "F_pitchrate_2p5m"

date = '_0423'

REPORT_PATH = f'./data_analysis/prelim/plots/report{date}.csv'
OUT_PATH    = f'./data_analysis/prelim/plots/box_plot_{METRIC}_{time.strftime("%m%d")}.png'
...
# Sort to lock in the color mapping and left-to-right grouping!
morphologies = sorted(report['morphology'].dropna().unique())

# You can also sort trials so the X-axis is always consistent 
# (Note: if you have mixed string/ints, you may need a custom sort here)
trials = sorted(report['trial'].dropna().unique())
...
for group_idx, group_name in enumerate(major_groups):
    ...
    for morph_idx, morph in enumerate(morphologies):
        ...
        y_data = report[mask][METRIC].dropna().values
        np.abs(y_data, out=y_data) # In-place absolute value
...
ax.set_ylabel(METRIC, fontsize=GLOB_FONTSIZE)
