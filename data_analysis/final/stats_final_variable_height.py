"""
Statistical Analysis (Extended, Variable Fall-Distance Target): Final Experiment

Same analysis as stats_final_v2.py (all 17 sections, verbatim), wrapped in
run_full_analysis(report_path, target_m, tag) and re-run once per available
report_final*.csv in this directory -- i.e. once per FALL_DISTANCE_TARGET_M that
data_analysis_final.py has been run with (see that file to generate more). Ends
with a cross-target comparison table for the four primary outcome coefficients,
using both the parametric and permutation-test p-values.

Run from this file's directory (data_analysis/final/).
"""

import os
import re

def run_full_analysis(report_path, target_m, tag):
    """
    Runs the full stats_final_v2.py analysis (all 17 sections) against one report CSV,
    tagging every saved figure with `tag` so repeated calls (one per height target) don't
    overwrite each other. Returns a small dict of the cross-target comparables; everything
    else (diagnostics, full model summaries, per-trial tables, permutation histograms,
    variance tests, TOST, ...) is printed/saved exactly as stats_final_v2.py does it.
    """
    # ============================================================================
    ...
    df = pd.read_csv(report_path, dtype={'date': str})
    ...
    plt.tight_layout()
    plt.savefig(f'diagnostic_initial_conditions_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'descriptive_outcomes_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'per_trial_heatmaps_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'per_trial_boxplots_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'effect_size_forest_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'residual_diagnostics_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'power_curve_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'permutation_histograms_{tag}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    def process_one_trial(file_path, date, trial, rep, morphology):
        ...
        t_2p5m = np.sqrt(2 * target_m / 9.81) + t0
    ...
    # ============================================================================
    # ## 17. Per-Target Summary
    #
    # stats_final_v2.py's Section 17 is a hand-written narrative interpretation of the 2.5m
    # baseline specifically (it cites exact numbers from that run) and wouldn't stay accurate
    # reused across arbitrary targets, so it's replaced here with a live summary, returned to
    # the driver below for the final cross-target comparison table instead of being printed
    # as fixed prose.
    # ============================================================================

    # NOTE: `models` (Section 5) and `outcomes`/`perm_results` (Sections 1/11) use two different
    # label strings for the same four outcomes ('|Roll Angle| (deg)' vs '|Roll Angle| at 2.5m
    # (deg)'). Build every entry below off `outcomes` + `var_to_model` so all four sub-dicts share
    # identical keys -- mixing label styles here caused a KeyError during testing.
    return {
        'tag': tag, 'target_m': target_m, 'report_path': report_path,
        'n_total': len(df),
        'n_with_tail': int((df['morphology'] == 'With Tail').sum()),
        'n_no_tail': int((df['morphology'] == 'No Tail').sum()),
        'coef':          {label: var_to_model[var].params[MORPH_PARAM] for var, label in outcomes},
        'ci':            {label: tuple(var_to_model[var].conf_int().loc[MORPH_PARAM]) for var, label in outcomes},
        'p_parametric':  {label: var_to_model[var].pvalues[MORPH_PARAM] for var, label in outcomes},
        'p_permutation': {label: perm_results[label][1] for var, label in outcomes},
    }



# =============================================================================
# Driver: discover every report_final*.csv in this directory, run the full analysis
# against each, then print a comparison table across fall-distance targets.
# =============================================================================

def discover_reports(directory='.'):
    """report_final.csv -> 2.5m (the baseline data_analysis_final.py writes by default);
    report_final_XpYm.csv -> X.Y m (whatever FALL_DISTANCE_TARGET_M produced it)."""
    found = []
    for fname in sorted(os.listdir(directory)):
        m = re.match(r'^report_final(?:_(\d+)p(\d+)m)?\.csv$', fname)
        if not m:
            continue
        if m.group(1) is None:
            target_m, tag = 2.5, '2p5m'
        else:
            target_m, tag = float(f"{m.group(1)}.{m.group(2)}"), f"{m.group(1)}p{m.group(2)}m"
        found.append((target_m, tag, os.path.join(directory, fname)))
    return sorted(found)

if __name__ == '__main__':
    reports = discover_reports()
    if not reports:
        raise SystemExit("No report_final*.csv files found in the current directory.")

    print("=" * 90)
    print(f"Found {len(reports)} report file(s) to sweep:")
    for target_m, tag, path in reports:
        print(f"  target={target_m}m  tag={tag}  path={path}")
    print("=" * 90)
    ...
    for target_m, tag, path in reports:
        print("\n\n")
        print("#" * 90)
        print(f"# RUNNING FULL ANALYSIS: target={target_m}m  (report: {path})")
        print("#" * 90)
        result = run_full_analysis(path, target_m, tag)
        all_results.append(result)

    # ── Cross-target comparison table ────────────────────────────────────────
    print("\n\n")
    print("=" * 100)
    print("CROSS-TARGET COMPARISON: does the fall-distance cutoff change the primary conclusion?")
    print("=" * 100)

    outcome_labels = list(all_results[0]['coef'].keys())
    for label in outcome_labels:
        print(f"\n{label}")
        print(f"  {'target':>8s} {'N(With/No)':>12s} {'coef':>9s} {'95% CI':>20s} "
              f"{'p (param)':>10s} {'p (perm)':>10s}")
        for r in all_results:
            coef = r['coef'][label]
            ci   = r['ci'][label]
            p_p  = r['p_parametric'][label]
            p_pm = r['p_permutation'][label]
            sig  = ' *' if p_p < 0.05 else ''
            print(f"  {r['tag']:>8s} {r['n_with_tail']:>5d}/{r['n_no_tail']:<6d} {coef:>+9.2f} "
                  f"[{ci[0]:7.2f},{ci[1]:7.2f}] {p_p:>10.4f} {p_pm:>10.4f}{sig}")

    print("\n* = p < 0.05 (parametric, uncorrected). Watch for sign flips and large coefficient")
    print("swings across targets, not just significance -- with N shrinking as the target grows")
    print("(stricter duration requirement excludes more trials), instability here is expected")
    print("from sample size alone and isn't automatically evidence the baseline target is wrong.")
