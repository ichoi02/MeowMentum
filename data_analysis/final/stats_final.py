"""
Final Experiment: Tail vs No-Tail Reorientation Comparison
============================================================
Main hypothesis test: does the tail improve reorientation performance
(less roll/pitch deviation and rate at the 2.5 m impact point)?

Stat approach mirrors the RL-vs-MBC controller comparison (stats_mbc.py):
  - Diagnostic check: does morphology assignment systematically bias initial states?
...
MORPHOLOGY_COLORS = {
    'With Tail': '#1A3A5C',   # dark navy
    'No Tail':   '#8B4513',   # saddle brown
}

REPORT_PATH = './data_analysis/final/report_final.csv'

# Bonferroni correction: 12 comparisons
# (3 roll conditions x 2 outcomes) + (3 pitch conditions x 2 outcomes)
N_COMPARISONS   = 12
ALPHA_CORRECTED = 0.05 / N_COMPARISONS   # ~0.00417
...
df = df[df['morphology'].isin(MORPHOLOGY_COLORS.keys())].copy()
...
print(pd.crosstab(df['morphology'], df['trial']))
...
print("Does tail presence systematically bias initial states?")
print("="*80)
...
for var, label in initial_vars:
    ...
    f_val = anova_table.loc['C(morphology)', 'F']
    p_val = anova_table.loc['C(morphology)', 'PR(>F)']
...
for var, label in outcome_vars:
    ...
    for cond in all_conditions:
        ...
        print(f"  {cond:15s}: WithTail={a.mean():6.1f}±{a.std():5.1f}  NoTail={b.mean():6.1f}±{b.std():5.1f}"
...
n_overall    = df[df['morphology'] == 'With Tail']['trial'].count() // len(all_conditions)
n_per_cond   = 6
...
for var, label in outcome_vars:
    a  = df[df['morphology'] == 'With Tail'][var].dropna().values
    b  = df[df['morphology'] == 'No Tail' ][var].dropna().values
...
PLOT_CONFIGS = [
    dict(
        ...
        ylim=YLIM_ANGLES,
        out='./data_analysis/final/boxplots_final_angles.png',
    ),
    dict(
        metrics=[
        ...
        ylim=YLIM_RATES,
        out='./data_analysis/final/boxplots_final_rates.png',
    ),
]
...
for cfg in PLOT_CONFIGS:
    ...
    fig.text(
        ...
        ('Significance markers: Bonferroni-corrected $t$-test '
         '($p_\\mathrm{adj} = p \\times 12$; $\\alpha = 0.05$; ns: not significant). '
         'N = 5–7 per group per condition (varies by condition); N ≈ 36 per group overall.'),
