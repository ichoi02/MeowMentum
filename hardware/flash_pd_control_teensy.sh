...
# the matching Teensy (SN_FRONT / SN_BACK from hardware/controllerV2.py).
#
...
print_sn_map() {
  ...
  while IFS=$'\t' read -r sn dev; do
    found=1
    role="(unknown SN)"
    [[ "$sn" == "$sn_front" ]] && role="FRONT (controllerV2 SN_FRONT)"
    [[ "$sn" == "$sn_back" ]] && role="BACK  (controllerV2 SN_BACK)"
    echo "  SN=${sn}  ${dev}  <- ${role}"
