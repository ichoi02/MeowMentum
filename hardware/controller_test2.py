...
# Gains (kept for reference; reduced controller uses its own gains)
Kp_spine = 1.0
Kp_settle_roll = 0.8
...
class TeensyInterface:
    def __init__(self, port_path, name):
        ...
        self.quat = [1.0, 0.0, 0.0, 0.0]       # w x y z (BODY frame after align)
        self.gyro = np.zeros(3, dtype=float)   # Gyro in BODY frame after align (gx, gy, gz)
        self.m1_rad = 0.0
        self.m2_rad = 0.0
        self.acc_mag = 0.0
    ...
    def update_sensor_data(self):
        ...
        if latest_line:
            ...
            if len(parts) == 10:
                try:
                    q_raw = [float(x) for x in parts[:4]]
                    # quat: sensor -> body (align)
                    self.quat = self.align_imu_quaternions(
                        np.array([q_raw]), self.name
                    ).tolist()
                    # gyro: sensor frame -> body frame (align with same r_align)
                    gyro_raw = np.array([float(x) for x in parts[4:7]], dtype=float)
                    self.gyro = self.align_imu_gyro(gyro_raw, self.name).tolist()
    ...
    def align_imu_gyro(self, gyro_sensor, imu_type):
        """Rotate angular velocity from IMU sensor frame to body frame.
 
        BNO08x gyro output is in the sensor's own frame. The quaternion alignment
        r_align = R_body_to_sensor rotates body-frame vectors INTO sensor-frame
        coords, so the inverse maps sensor-frame angular velocity back to body:
            omega_body = R_align^{-1} @ omega_sensor
        """
        if imu_type == 'Front':
...
######## Helpers kept from original file (handy for debug/logging) ########
...
def compute_derived_state(state):
    ...
    phi_coupl = 0.5 * (front_roll_sign * qf + rear_roll_sign * qr)
    phi_diff = (front_roll_sign * qf - rear_roll_sign * qr)
...
class ReducedModelController:
    ...
    def compute(self, front, back, dt):
        ...
        self._q_prev    = q_j.copy()
        # (qdot_filt available for future use; not currently needed by the law)
...
def main():
    ap = argparse.ArgumentParser(description="Teensy Control Loop with Telemetry Logging")
    ap.add_argument(
        '--debug',
        action='store_true',
    ...
    try:
        while True:
            ...
            if args.debug:
                # debug 모드: FSM 바이패스, 키보드로 action 직접 조작
            ...
            # --- Common tail: send commands, print, log, enforce timing ---
            phase_log = phase
